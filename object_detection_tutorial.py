...
def moveVideoToArchive(file):
    shutil.move(file, "/data/videos/archive")
...
def notifySlack(plt, image_np):
    ...
    with open(file, 'rb') as f:
        sc.api_call(
            ...
            channels=channel_id,
            filename='$file',
            title='Video where unidentified person was detected',
            initial_comment='Video where unidentified person was detected',
            file=io.BytesIO(f.read())
        )
...
while True:
  ...
  file = wait_for_video( "/data/videos/incoming", 1, 1 ) 
